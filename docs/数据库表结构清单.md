...
| 表名 | 中文名称 | 功能描述 | 主要字段 |
|------|----------|----------|----------|
| `map_sn` | SN映射表 | SN到批次/工单/箱号/托盘映射 | sn, lot_id, wo_id, box_no, pallet_no |
| `map_box_sn` | 箱号SN映射表 | 箱号到SN映射 | box_no, sn |
| `map_pallet_box` | 托盘箱号映射表 | 托盘到箱号映射 | pallet_no, box_no |
| `map_lot_material` | 批次用料映射表 | 批次到用料来源映射 | lot_id, item_id, supplier_id, grn_id |

### 1.8 事件溯源表 (2张)

| 表名 | 中文名称 | 功能描述 | 主要字段 |
|------|----------|----------|----------|
| `trace_event` | 追溯事件表 | 所有可追节点抽象为不可变事件 | event_id, entity_type, entity_id, action, occurred_at |
| `trace_link` | 追溯链路快照表 | 链路快照(原材料/组件/成品) | level, sn, lot_id, wo_id, op_id, result |

### 1.9 测试和测量表 (2张)

| 表名 | 中文名称 | 功能描述 | 主要字段 |
|------|----------|----------|----------|
| `test_record` | 测试记录表 | 产品测试结果记录 | sn, lot_id, station, result, code, tested_at |
| `measure_record` | 测量记录表 | 检验测量数据记录 | insp_id, item_key, value_num, standard_value, measured_at |
