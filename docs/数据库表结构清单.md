...
| `site` | 站点管理表 | 工厂站点管理 | site_id, site_code, site_name |
| `sys_user` | 用户表 | 系统用户管理 | user_id, username, email, phone |
| `sys_role` | 角色表 | 角色定义 | role_id, role_name, role_type |
| `sys_permission` | 权限表 | 权限定义 | permission_id, permission_code |
| `sys_menu` | 菜单表 | 系统菜单 | menu_id, menu_name, menu_url |
| `sys_user_role` | 用户角色关联 | 用户角色分配 | user_id, role_id |
| `sys_role_permission` | 角色权限关联 | 角色权限分配 | role_id, permission_id |
| `employee` | 员工表 | 员工基础信息 | employee_id, employee_no, real_name |
